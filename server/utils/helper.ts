...
//     return (allowEmpty || !CommonHelper.isEmpty(arr)) &&
...
//     const alphabet =
//       "-_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"; // 64 to devide "cleanly" 256
...
//       separator.length > 1 ? separator.trim() : separator;
...
//     filename = filename || "";
//     return !!imageExtensions.find((ext) =>
...
//     filename = filename || "";
//     return !!videoExtensions.find((ext) =>
//       filename.toLowerCase().endsWith(ext)
//     );
//   }
...
//     filename = filename || "";
//     return !!audioExtensions.find((ext) =>
//       filename.toLowerCase().endsWith(ext)
//     );
//   }
...
//     filename = filename || "";
//     return !!documentExtensions.find((ext) =>
//       filename.toLowerCase().endsWith(ext)
//     );
//   }
...
//     const hasCreated =
//       !isView ||
//       CommonHelper.extractColumnsFromQuery(collection?.options?.query).includes(
//         "created"
//       );
//     if (hasCreated) {
...
//     // arrayable fields
//     if (
//       ["select", "relation", "file"].includes(field?.type) &&
...
//       if (
//         prop !== "schema" &&
//         JSON.stringify(oldCollection[prop]) !==
//           JSON.stringify(newCollection[prop])
//       ) {
//         return true;
//       }
//     }

//     const oldSchema = Array.isArray(oldCollection.schema)
//       ? oldCollection.schema
//       : [];
//     const newSchema = Array.isArray(newCollection.schema)
//       ? newCollection.schema
//       : [];
//     const removedFields = oldSchema.filter((oldField) => {
//       return (
//         oldField?.id && !CommonHelper.findByKey(newSchema, "id", oldField.id)
//       );
//     });
//     const addedFields = newSchema.filter((newField) => {
//       return (
//         newField?.id && !CommonHelper.findByKey(oldSchema, "id", newField.id)
//       );
//     });
//     const changedFields = newSchema.filter((newField) => {
//       const oldField =
//         CommonHelper.isObject(newField) &&
//         CommonHelper.findByKey(oldSchema, "id", newField.id);
//       if (!oldField) {
//         return false;
//       }
...
//       try {
//         return (
//           CommonHelper.truncate(JSON.stringify(val), truncateLength) ||
//           missingValue
//         );
//       } catch (_) {
//         return missingValue;
...
//       for (let col of CommonHelper.extractColumnsFromQuery(
...
//    * @return {Array}
//    */
//   static getCollectionAutocompleteKeys(
//     collections,
//     collectionNameOrId,
//     prefix = "",
//     level = 0
...
//       if (
//         field.options?.maxSelect != 1 &&
//         ["select", "file", "relation"].includes(field.type)
//       ) {
...
//       (collection) => collection.type === "auth"
//     );
...
//       const colRegex =
//         /^([\s\S]+?)(?:\s+collate\s+([\w]+))?(?:\s+(asc|desc))?$/gim;
...
//     result += `\`${
//       indexParts.indexName || "idx_" + CommonHelper.randomString(7)
//     }\` `;
